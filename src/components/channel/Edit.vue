...
export default {
  ...
  created () {
    if (!this.$store.state.channel_edit) {
      this.$store.registerModule('channel_edit', modelModule)
    }
  ...
  methods: {
    async setModel () {
      await this.$store.dispatch('channel_edit/fetch', {
    ...
    async onSubmit () {
      ...
      if (
        ...
        // Update model
        await this.$store.dispatch('channel_edit/update', {
    ...
    async onDelete () {
      // Delete model
      await this.$store.dispatch('channel_edit/remove', {
        path: 'channel/' + this.data.id
      })
    ...
    async refresh () {
      await this.$store.dispatch('channel_edit/refresh')
